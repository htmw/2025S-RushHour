...
import {
    ...
    DialogContent, DialogActions, Divider, Avatar, CircularProgress,
} from "@mui/material";
        ...
        <Box mt={4} ml={4}>
                            ...
                            <Box display="flex" alignItems="center" gap={2}>
                                ...
                                <Box>
                                    ...
                                    <Typography fontSize="0.85rem" color="text.secondary">
                                        Age: {patient.patientProfile?.age || "-"} / Sex: {patient.patientProfile?.sex || "-"}
                                    </Typography>
                                </Box>
                            </Box>
                            <Button
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="md">
                ...
                <DialogContent dividers>
                    {patientDetails ? (
                        <>
                            <Typography variant="h6">{patientDetails.user.fullName}</Typography>
                            <Typography variant="body2" color="text.secondary">{patientDetails.user.email}</Typography>
                            <Divider sx={{ my: 2 }} />
                            <Box mt={3}>
                                ...
                                {patientDetails.medicalHistory.length > 0 ? (
                                    patientDetails.medicalHistory.map((entry, idx) => (
                                        <Paper key={idx} sx={{ p: 2, mb: 1, bgcolor: "#f9f9f9" }}>
                                            <Typography fontWeight={600}>{entry.healthIssue}</Typography>
                            ...
                            <Box mt={3}>
                                ...
                                {patientDetails.insurance ? (
                                    <>
                                        <Typography>Provider: {patientDetails.insurance.providerName}</Typography>
                                        <Typography>
                                            Valid Till: {new Date(patientDetails.insurance.endDate).toLocaleDateString()}
                                        </Typography>
                                    </>
                                ) : (
                                    <Typography variant="body2" color="text.secondary">
