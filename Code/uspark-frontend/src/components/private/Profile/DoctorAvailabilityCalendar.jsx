...
import {
    ...
    FormControlLabel, Card, CardContent, CardHeader, Pagination
} from "@mui/material";
...
import { fetchDoctorAvailability, saveDoctorAvailability, updateDoctorAvailability } from "../../../store/actions";
...
const DoctorAvailabilityCalendar = () => {
    ...
    const filteredAvailability = availability.filter((slot) => {
        ...
        const isAfterStart = filterStart ? slotDate.isSameOrAfter(dayjs(filterStart), "day") : true;
        const isBeforeEnd = filterEnd ? slotDate.isSameOrBefore(dayjs(filterEnd), "day") : true;
    ...
    const isSlotBooked = (slot) => {
        return appointments?.some(appt =>
            appt.date === slot.date &&
            dayjs(appt.startTime, "HH:mm").isSameOrAfter(dayjs(slot.startTime, "HH:mm")) &&
            dayjs(appt.startTime, "HH:mm").isBefore(dayjs(slot.endTime, "HH:mm"))
        );
    };
    const bookedAppointment = appointments.find(appt =>
    ...
    const getDatesBetween = (start, end) => {
        ...
        while (current.isBefore(end) || current.isSame(end, "day")) {
            if (includeWeekends || (!includeWeekends && current.day() !== 0 && current.day() !== 6)) {
        ...
        <>
                    ...
                    {availability.length === 0 ? (
                        ...
                        <Stack spacing={2}>
                            ...
                            {filteredAvailability.length === 0 ? (
                                <Typography>No availability found for selected dates.</Typography>
                            ) : (
                                        ...
                                        <Grid item xs={12} sm={6} md={4} key={index}>
                                            <Card
                                                sx={{
                                                    ...
                                                    backgroundColor: isSlotBooked(slot) ? "#fff8dc" : "#f9f9fb",
                                                    borderLeft: isSlotBooked(slot) ? "5px solid #ffc107" : "5px solid #1976d2",
                                            ...
                                            >
                                                <IconButton
                                                    ...
                                                    onClick={() => {
                                                        setEditingSlot(slot);
                                                        setEditedStartTime(dayjs(`2025-01-01T${slot.startTime}`));
                                                        setEditedEndTime(dayjs(`2025-01-01T${slot.endTime}`));
                                                ...
                                                <CardContent sx={{ pb: "12px !important" }}>
                                                    ...
                                                    <Typography variant="body2" sx={{ mt: 1, fontSize: "0.85rem", color: "#555" }}>
                                                        <TimelapseIcon fontSize="small" sx={{ mr: 0.5, verticalAlign: "middle" }} />
                                                        Slot Duration: <strong>{slot.slotDuration} minutes</strong>
                                                    </Typography>
                                                </CardContent>
                                            </Card>
                                        </Grid>
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers sx={{ maxHeight: "75vh", overflowY: "auto" }}>
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <FormControlLabel
                                    control={<Checkbox checked={includeWeekends} onChange={(e) => setIncludeWeekends(e.target.checked)} />}
                ...
                <DialogActions>
                    ...
                    <Button variant="outlined" onClick={handleGenerateSlots}>Generate</Button>
                    <Button variant="contained" onClick={handleApprove}>Approve & Save</Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="sm" fullWidth>
                ...
                <DialogContent dividers>
                    {bookedAppointment && (
                        <Paper elevation={1} sx={{ p: 2, mb: 2, backgroundColor: "#fff8dc" }}>
                            <Typography variant="subtitle1" fontWeight="bold">Appointment Info</Typography>
                            <Typography variant="body2"><strong>Patient:</strong> {bookedAppointment.name}</Typography>
                            <Typography variant="body2"><strong>Email:</strong> {bookedAppointment.email}</Typography>
                            <Typography variant="body2"><strong>Reason:</strong> {bookedAppointment.reason}</Typography>
                            <Typography variant="body2"><strong>Time:</strong> {bookedAppointment.startTime}</Typography>
                        </Paper>
                    )}
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                        <Grid container spacing={2}>
