...
const MedicalHistory = ({}) => {
  const medicalHistories = useSelector(
    (state) => state.medicalHistory?.histories || []
  );
  ...
  return (
    <Paper elevation={3} sx={{ p: 3, mt: 3 }}>
        ...
        <TableBody>
          {medicalHistories.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {moment(item.dateOfOccurrence).format("YYYY-MM-DD")}
              </TableCell>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
        <DialogTitle>
          {editIndex !== null ? "Edit" : "Add"} Medical History
        </DialogTitle>
        <DialogContent>
          <Stack spacing={2} mt={1}>
            ...
            <FileUpload
              ...
              onFilesChange={(uploaded) =>
                setFormData({ ...formData, attachments: uploaded })
              }
              inputProps={{ "data-cy": "medical-file-upload" }}
            />
          </Stack>
        </DialogContent>
        <DialogActions>
